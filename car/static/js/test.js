 ...
 */
var extensionId = "helkffleffnhhnoepaimiaebdcagpllc";
...
function SerialPort(){
    ...
    this.write = function(data, callBack){
        let format_data = new Uint8Array(data);
        let arr_data= [];
        // console.log(format_data)
        for (let i =0; i<format_data.length;i++){
            arr_data.push(format_data[i])
        }
        arr_data.push(13);
        arr_data.push(10);
        chrome.runtime.sendMessage(extensionId,
            {
                cmd: "write",
                connectionId: serialConnectionId,
                data: Array.prototype.slice.call(new Uint8Array(arr_data))
            },
            function(response){
                if(response.result === "ok"){
